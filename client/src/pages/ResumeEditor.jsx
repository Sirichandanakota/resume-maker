...
import { User, Mail, Phone, MapPin, Link2, Plus, Trash2, FileText, ArrowLeft, Image as ImageIcon, Download, GripVertical, Eye, EyeOff, AlertTriangle, Undo2, Redo2, X, ShieldAlert, Award, CheckCircle, LogOut, ChevronRight, Lock } from 'lucide-react';

// ==========================================
// 1. CONSTANTS & DEFAULTS (Must be at top)
// ==========================================
...
// ==========================================
// 2. MOCKUPS & UI COMPONENTS (Hoisted Functions)
// ==========================================
...
function ResumeEditor({ template, userFullName, userEmail, onBack }) {
  ...
  const restoreState = (stateStr) => {
    ...
    try {
      ...
      setFontSizeNum(s.settings.fontSizeNum || (template === '1-column' ? 9 : 12)); 
      setFontFamily(s.settings.fontFamily || "'Times New Roman', serif");
      setThemeColor(s.settings.themeColor || '#31414e'); 
      setThemeTextColor(s.settings.themeTextColor || (template === '1-column' ? 'black' : 'white')); 
      setHeadSizeSelection(s.settings.headSizeSelection || (template === '1-column' ? '0' : '32')); 
  ...
  const renderTimelineItem = (content, id, timeline, mode, gapMb = 'mb-3', gapPb = 'pb-3') => {
    ...
    if (timeline) {
      return <div key={id} className={`relative pl-4 border-l ${gapPb} last:pb-0`} style={{ borderLeftColor: bColor }}><div className="absolute w-2 h-2 rounded-full -left-[4.5px] top-1.5" style={{ backgroundColor: dColor }}></div>{content}</div>;
    }
    ...
    if (id.startsWith('custom')) {
      ...
      return (
        <section key={id} className={`${sectionMb}`} style={style}>
          ...
          <div className="space-y-0">{(d.items || []).map(i => (i.title || i.subtitle || i.description) && renderTimelineItem(<div className="block">{i.title && <h3 style={{...sLg, color: hColor}} className="font-bold">{i.title}</h3>}{i.subtitle && <div style={{...sSm, color: pColor}}>{i.subtitle}</div>}{i.description && renderDescription(i.description, i.isBullet, pColor, sBase)}</div>, i.id, section.timeline, mode))}</div>
        </section>
      );
    }
  ...
  return (
    <div onContextMenu={(e) => e.preventDefault()} className="flex flex-col lg:flex-row h-screen bg-gray-100 overflow-hidden font-sans relative select-none">
      {/* SCREENSHOT BLACKOUT OVERLAY */}
      {isObscured && (
        <div className="absolute inset-0 z-[99999] bg-black flex flex-col items-center justify-center text-white">
          <ShieldAlert size={64} className="text-red-500 mb-4" />
          <h1 className="text-3xl font-bold mb-2">Action Disabled</h1>
          <p className="text-gray-400">Screenshots and saving are disabled for this editor.</p>
        </div>
      )}
        ...
        <div className="w-full max-w-[816px] flex flex-col gap-4 pb-12">
          ...
          <div className="bg-white rounded-xl shadow p-4 border border-gray-200 shrink-0">
            <div className="flex items-center gap-2 mb-4 border-b pb-4">
              <button onClick={undo} disabled={historyIndex<=0} className="p-2 bg-slate-100 rounded disabled:opacity-50 hover:bg-slate-200 transition-colors"><Undo2 size={18}/></button>
              <button onClick={redo} disabled={historyIndex>=historyRef.current.length-1} className="p-2 bg-slate-100 rounded disabled:opacity-50 hover:bg-slate-200 transition-colors"><Redo2 size={18}/></button>
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider ml-2">History</span>
            </div>
          ...
          <div className="w-full overflow-x-auto text-center pb-12 select-none">
...
// ==========================================
// 3. MAIN APP (Must be at the very bottom)
// ==========================================
export default function App() {
  const [authState, setAuthState] = useState('home'); 
