...
export default function LoginPage({ onLogin, onSwitchToSignUp, onBack }) {
  ...
  // 1. Initialize error from localStorage so it stays after a refresh
  const [error, setError] = useState(() => localStorage.getItem('RM_login_error') || '');
  const [loading, setLoading] = useState(false);

  // 2. Persistent Error Effect: Saves the error message whenever it changes
  useEffect(() => {
    if (error) {
      localStorage.setItem('RM_login_error', error);
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response?.data?.token) {
        ...
        // 3. SUCCESS: Clear the persistent error before navigating
        setError(''); 
        localStorage.removeItem('RM_login_error');
      ...
      // 4. FAILURE: Set error. The useEffect above will immediately save it to localStorage.
  ...
  return (
    <div className="h-screen w-full flex font-sans bg-white relative overflow-hidden">
      <button 
        type="button" 
        onClick={() => {
           setError(''); // Clear error if they choose to go back
           onBack();
        }} 
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-8 bg-slate-50">
        <div className="w-full max-w-md bg-white p-6 sm:p-10 rounded-3xl shadow-xl border border-slate-100 mt-10 lg:mt-0">
          ...
          {/* THE ERROR: This will now stay visible even after a page reload */}
          {error && (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button 
              type="submit" 
              disabled={loading} 
              className="w-full bg-blue-600 hover:bg-blue-700 text-white font-extrabold py-3.5 rounded-xl flex items-center justify-center gap-2 transition-all mt-4 shadow-lg disabled:opacity-70 disabled:cursor-wait"
            >
